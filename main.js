...
new THREE.TextureLoader(loadMgr).load('./data/env_equirect.png', tex => {
  ...
  tex.dispose();
}, undefined, () => console.warn('env_equirect.png missing — continuing without IBL'));
...
// ---------- resilient texture fallback ----------
// A missing texture must never throw, never stall the LoadingManager, and
// never leave a black/invisible mesh: swap in a small procedural canvas
// matched to the map type.
function fallbackCanvas(kind) {
  const c = document.createElement('canvas'); c.width = c.height = 64;
  const x = c.getContext('2d');
  if (kind === 'normal') {                       // flat tangent-space normal
    x.fillStyle = '#8080ff'; x.fillRect(0, 0, 64, 64);
  } else if (kind === 'ao') {                    // no occlusion
    x.fillStyle = '#ffffff'; x.fillRect(0, 0, 64, 64);
  } else if (kind === 'ms') {                    // neutral metallic/smoothness
    x.fillStyle = '#808080'; x.fillRect(0, 0, 64, 64);
  } else if (kind === 'emissive') {              // soft radial glow / smoke puff
    const g = x.createRadialGradient(32, 32, 2, 32, 32, 32);
    g.addColorStop(0, 'rgba(255,255,255,1)');
    g.addColorStop(0.55, 'rgba(150,150,150,0.55)');
    g.addColorStop(1, 'rgba(0,0,0,0)');
    x.fillStyle = g; x.fillRect(0, 0, 64, 64);
  } else {                                       // albedo: dark neutral gray + subtle noise
    x.fillStyle = '#3a3d42'; x.fillRect(0, 0, 64, 64);
    const d = x.getImageData(0, 0, 64, 64), px = d.data;
...
function texKind(file) {
  const f = file.toLowerCase();
  if (/_n\.|_normal\.|_normal_|normals/.test(f)) return 'normal';
  if (/_ao\.|_ao_|ambientocclusion/.test(f)) return 'ao';
  if (/_ms\.|_ms_|metallicsmoothness/.test(f)) return 'ms';
  if (/_e\.|emis|smoke|glow/.test(f)) return 'emissive';
  return 'albedo';
}
function applyTexFallback(t, file) {
  t.image = fallbackCanvas(texKind(file));
  t.needsUpdate = true;
  console.warn('texture missing, procedural fallback used:', file);
}
...
function texture(file, srgb = true) {
  ...
  if (!texCache[key]) {
    const t = new THREE.TextureLoader(loadMgr).load(
      './textures/' + encodeURIComponent(file),
      undefined, undefined,
      () => applyTexFallback(t, file));
...
function packMetalRough(file, onReady) {
  ...
  const img = new Image();
  img.onload = () => pack(img, img.width, img.height);
  img.onerror = () => {
    // neutral: metal 0, smoothness ~0.5 -> roughness ~0.5 (never black/mirror)
    console.warn('MS texture missing, neutral fallback:', url);
    const fb = document.createElement('canvas'); fb.width = fb.height = 4;
    const fx = fb.getContext('2d');
    fx.fillStyle = 'rgba(0,0,0,0.5)'; fx.fillRect(0, 0, 4, 4);
    pack(fb, 4, 4);
  };
  img.src = url;
}
...
function charTexture(file, srgb = true) {
  ...
  if (!charTexCache[file]) {
    const t = new THREE.TextureLoader(loadMgr).load(
      './chars/textures/' + encodeURIComponent(file),
      undefined, undefined,
      () => applyTexFallback(t, file));
    t.flipY = false;
    t.colorSpace = srgb ? THREE.SRGBColorSpace : THREE.LinearSRGBColorSpace;
...
function charPackMetalRough(file, onReady) {
  ...
  img.onload = () => pack(img, img.width, img.height);
  img.onerror = () => {
    console.warn('char MS texture missing, neutral fallback:', file);
    const fb = document.createElement('canvas'); fb.width = fb.height = 4;
    const fx = fb.getContext('2d');
    fx.fillStyle = 'rgba(0,0,0,0.5)'; fx.fillRect(0, 0, 4, 4);
    pack(fb, 4, 4);
...
import { buildCityBoxes } from './cityBoxes.js?v=8';
import { Controller } from './controller.js?v=8';
import { CameraRig } from './cameraRig.js?v=8';
